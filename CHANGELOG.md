...
- 修复遗留 Manager 数据库维护事务被普通恢复分派误判为 `AmbiguousPendingTransactions`、阻塞后续升级的问题；`upgrade` 与公开 `recover` 现在通过维护执行器的仅恢复入口结束旧事务，再继续原流程，不提前执行新一轮清理。
- 已到 `validated` 的维护事务重新校验当前数据库后提交，避免还原旧备份覆盖后续新增数据；更早阶段保留备份恢复，并继续拒绝混杂事务、归属不一致的记录、异常备份和失败的数据校验。
- 新增维护恢复分派、仅恢复、重复调用及失败门禁回归，纳入快速与完整检查，并同步 Skill 和故障排查说明。已完成正式实例的遗留维护事务恢复、CPA/Manager 升级和升级后数据库维护验证。

## 1.3.3 - 2026-09-21

- 修复完整启动流程在服务已健康后读取 Fast 模式专属 `PreviousProcessId` 字段，导致维护重启被误报为失败并进入回滚的问题；普通启动现在稳定输出 `null`，Fast + Restart 继续保留旧进程 PID。
- 新增启动结果契约回归，覆盖普通与 Fast 两种结果形状，并纳入快速和完整检查入口。

## 1.3.2 - 2026-09-21

- updater 检查固定官方 GitHub API 时，自动复用当前 Windows 用户已登录的 GitHub CLI 凭据；凭据仅在内存中读取，且每个重定向重新判断目标，不会发送给 Release 资产或其他 HTTPS/HTTP 目标。
- 没有 GitHub CLI 或凭据不可用时保持匿名请求；凭据被 GitHub 拒绝时不静默降级重试。下载失败保留 `httpStatus`、限流余量、重置时间和 `Retry-After` 数值诊断，不输出响应正文或凭据。
- 新增认证边界与诊断脱敏回归，并更新 Skill 文档说明登录依赖、匿名降级和限流处理。

## 1.3.1 - 2026-09-21

- 桌面 `CPA 本地启动.lnk` 改为 Fast + Restart 契约：先按配置路径停止现有 CPA/Manager 进程，再重新启动并打开管理页面；CLI `start`、升级与恢复路径仍保持原有复用和安全检查行为。
- 快速启动输出显示停止/启动阶段的颜色区别，并在停止阶段标出旧 PID 与端口，摘要显示当前 PID、端口和最终动作；进程匹配仍严格限制为配置中的可执行文件完整路径。

## 1.3.0 - 2026-09-21

- **兼容性变化**：仅支持 Windows 上的 PowerShell 7（`pwsh.exe`），移除 PS5 启动回退和双宿主 CI；自动化及手工命令需要使用 PS7。
- **功能移除**：删除 LAN 管理命令及其专属实现和测试，不修改现有实例的网络绑定。旧版遗留 LAN 事务仍会阻断，不能静默忽略。
- 精简日常原地升级：直接使用校验后的官方发行包，不再复制凭据、分配候选端口或预跑候选服务；首次迁移仍保留候选验证。正式 config/auth 原地保留，切换后探活失败则回滚对应组件。
- 成功提交只归档已验证备份，不重复重启健康服务或执行整栈恢复检查；保留旧格式中断事务恢复、备份校验及脱敏诊断。
- 修复 CPA 回滚中断后的恢复状态判断，并持久化关键阶段；日常检查不再递归扫描 `auth/logs` 历史日志，凭据和日志目录边界仍受检查。
- Manager 旧程序备份移到停服前；数据库与 `data.key` 仍在停服后一致性备份。只更新 CPA 时不进入 Manager 备份流程，正常保留最近一份回滚备份。
- 对明确的本机 `/setup` SQLite 忙错误做有界重试，其他失败仍立即报告；共享原生进程启动、配置辅助函数与子进程执行逻辑，同进程调用改用原生参数展开。
- 默认及 CI 仅执行 PS7 短检查；安装、迁移和长时间故障注入测试需显式指定 `-Extended`。已完成隔离真实升级，以及正式实例的遗留事务恢复、升级和数据库维护验证。

## 1.2.2 - 2026-09-19

- 升级结果按时间保留脱敏的预检、切换后检查、恢复检查与异常位置，明确列出失败条件及允许的本机接口状态；不记录密钥、请求与响应正文、完整 URL、动态候选端口或数据库内容。
- Manager 候选验证把接口、HTTP 状态或传输失败跨进程内异常边界带入最终诊断，首次失败不会被后续恢复成功覆盖。
- `changed` 与 `recovered` 结果汇总反映已经成功切换的组件和内部恢复，避免整体失败时误报为没有任何变更或恢复。
- 新增 Windows PowerShell 5.1 / PowerShell 7 脱敏与结果契约回归，并覆盖切换健康和候选硬中断恢复路径。
...
- 回滚槽轮换统一使用同一受限重试原语，并增加瞬时成功、持续拒绝和非瞬时失败回归测试。
