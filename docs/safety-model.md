...
所有投影目录、文件、JSON 临时后缀和目录交换后缀都必须满足 Windows 路径预算：目录不超过 247 字符，文件不超过 259 字符。初始化和升级会在停止第一个正式服务或禁用 Manager collector 前完成预检；超限时不进入停机窗口。

日常状态、启动、原地升级和恢复把 `auth/logs` 视为日志边界：检查该目录本身的 owner、ACL 和 reparse 属性，但不遍历其历史日志与临时文件。真实凭据（含其他子目录）仍递归检查；复制 auth 时不复制 `logs`。通用目录审计与首次接管加固仍保留完整递归行为，不把这一例外推广到 plugins 或 Manager 数据库目录。

## Release 信任

Release 元数据与资产只从硬编码的官方 GitHub 仓库通过 HTTPS 获取。CPA/Manager 压缩包 SHA256 必须匹配 `checksums.txt`；GitHub 提供 asset digest 时也会校验。解压前检查 ZIP 条目、路径、数量和总大小。exe hash 会贯穿下载和正式切换；首次迁移另做候选验证。

上游 Windows executable 不假设拥有 Authenticode 签名。实际信任边界是上游 GitHub 仓库与已验证 release hash。

若旧 binary 的版本或来源无法可靠识别，公开 `upgrade` 自动允许用已验证的 latest stable 替换。该策略只放宽版本单调性证明；官方 release、checksum、切换后健康、SQLite 水位、进程身份与自动回滚门禁保持不变。
...
无密钥 pending journal 在破坏性动作前原子写入，并绑定 instance ID。`recover` 只调用底层 recovery-only interface；即使并发恢复时 journal 已被另一调用收敛，也不会开始新迁移、联网升级。初始化或升级的顶层 journal 可以拥有经过底层再次校验的 switch journal 与 `rollback/pending-*` 从属 artifact；无关的多个顶层事务仍视为歧义。runtime 切换与 `current.json` 提交是可恢复的两阶段提交：

- recorded hash 仍是 old 时，幂等重铺已验证 old runtime/Manager 数据；
- recorded hash 与 active hash 都是 new 时，完成 last-known-good 提交；
- 其他组合视为歧义并停止，不猜测恢复。

日常原地升级不启动候选实例、不复制 auth 到测试目录。下载校验、路径和备份检查在停服前完成；组件替换后立即探活，失败回滚。成功只归档备份，不重复整栈扫描。首次迁移仍保留候选验证。各组件独立提交/回滚，不撤销另一组件已经成功的升级。
...
候选使用动态分配、未占用的高位端口，必须只绑定 `127.0.0.1`，并检查同一端口的全部 listener。候选端口不属于公开接口或固定配置。新实例默认 loopback；本工具不再提供网络暴露切换命令，也不修改既有实例的绑定配置。旧版遗留 LAN 事务必须用兼容版本恢复，新版不会静默忽略。
