...
普通非空目录不能被认领。installer 只可对用户明确指定、ACL 已保护且完全为空的 root 写入带 bootstrap hash 的 preinitialized instance marker；此状态只允许固定 launcher，直到单独授权的迁移建立同 instance 的 `current.json`。任何额外内容、marker 漂移或 launcher hash/ACL 漂移都会阻断。
